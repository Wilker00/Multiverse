    ...
    try:
        for ep in range(int(episodes)):
            ...
            if not bool(failures_only) or not bool(ep_success):
                # print(f"DEBUG: Extending rows with {len(ep_rows)} from ep {ep}, mismatch_only={mismatch_only}")
                rows.extend(ep_rows)
...
def main() -> None:
    ...
    ap.add_argument("--expert_warmup_episodes", type=int, default=500)
    ap.add_argument("--expert_refit_episodes", type=int, default=50)
    ...
    ap.add_argument("--expert_policy", type=str, default="planner", choices=["q", "planner", "hybrid"])
