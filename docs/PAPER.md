...
2. result: `314 collected` for the current repo-local test selection (`python -m pytest tests tests/test_dt_memory.py --collect-only -q` on March 3, 2026); latest archived full pass for the February 22, 2026 paper snapshot remains `232 passed`
...
| --- | --- | --- |
| Framework currently collects 314 repo-local pytest cases; latest archived paper snapshot passed 232 tests | Confirmed | `python -m pytest tests tests/test_dt_memory.py --collect-only -q` (March 3, 2026) and `.venv312/Scripts/python.exe -m pytest -q` (February 22, 2026 snapshot) |
