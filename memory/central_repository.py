...
def find_similar_batch(
    *,
    ...
    - Reusing filtering and normalization operations

    Args:
        observations: List of observations to query (batch size)
        cfg: Central memory configuration
    ...
    # Small batches: use sequential (overhead not worth it)
    if len(observations) <= 2:
        return [
            find_similar(
                obs=obs, cfg=cfg, top_k=top_k, verse_name=verse_name,
                min_score=min_score, exclude_run_ids=exclude_run_ids,
                decay_lambda=decay_lambda, current_time_ms=current_time_ms,
                memory_tiers=memory_tiers, memory_families=memory_families,
                memory_types=memory_types, stm_decay_lambda=stm_decay_lambda,
    ...
    all_results: List[List[ScenarioMatch]] = [[] for _ in range(len(observations))]

    for mem_path in mem_paths:
        if not os.path.isfile(mem_path):
            continue

        # Load cache ONCE for entire batch
        cache = _get_similarity_cache_for_path(mem_path=mem_path, tier_policy=tier_policy)

        # Group queries by dimension
        queries_by_dim: Dict[int, List[Tuple[int, List[float]]]] = {}
        for i, qvec in enumerate(query_vecs):
            q_dim = len(qvec)
            if q_dim not in queries_by_dim:
                queries_by_dim[q_dim] = []
            queries_by_dim[q_dim].append((i, qvec))

        # Batch process each dimension group
        for q_dim, query_group in queries_by_dim.items():
            if q_dim == 0 or q_dim not in cache.vectors_by_dim or np is None:
                # Fallback to sequential
                for query_idx, _ in query_group:
                    result = find_similar(obs=observations[query_idx], cfg=cfg, top_k=top_k,
                                        verse_name=verse_name, min_score=min_score,
                                        exclude_run_ids=exclude_run_ids, decay_lambda=decay_lambda,
                                        current_time_ms=current_time_ms, memory_tiers=memory_tiers,
                                        memory_families=memory_families, memory_types=memory_types,
                                        stm_decay_lambda=stm_decay_lambda, trajectory_window=trajectory_window)
                    all_results[query_idx].extend(result)
                continue

            try:
                # Batch matrix multiplication
                Q = np.array([qvec for _, qvec in query_group], dtype=np.float32)
                Q_norms = np.linalg.norm(Q, axis=1, keepdims=True)
                Q_normalized = Q / np.maximum(Q_norms, 1e-12)
                memory_vectors = cache.vectors_by_dim[q_dim]
                similarities = Q_normalized @ memory_vectors.T  # (n_queries × n_memories)

                dim_row_idxs = list(cache.row_indices_by_dim.get(q_dim, []))

                # Process each query's results
                for batch_idx, (query_idx, _) in enumerate(query_group):
                    sims_for_query = similarities[batch_idx]
                    scored: List[Tuple[float, ScenarioMatch]] = []

                    for pos, row_idx in enumerate(dim_row_idxs):
                        row = cache.rows[int(row_idx)]
                        ...
                        if exclude_run_ids and str(row.run_id) in exclude_run_ids:
                            continue
                        if tier_filter and row.row_tier not in tier_filter:
                            continue
                        if family_filter and row.row_family not in family_filter:
                            continue
                        if type_filter and row.row_type not in type_filter:
                            continue

                        raw_score = float(sims_for_query[pos])
                        t_ms = int(row.t_ms)
                        ...
                        score, recency_weight = apply_temporal_decay(
                            score=float(raw_score), t_ms=t_ms,
                            decay_lambda=float(row_decay_lambda),
                            current_time_ms=current_time_ms
                        )
                    ...
                    all_results[query_idx].extend([m for _, m in scored[:top_n]])

            except Exception as exc:
                import logging
                logging.warning(f"Batch processing failed for dim {q_dim}: {exc}. Fallback to sequential.")
                for query_idx, _ in query_group:
                    result = find_similar(obs=observations[query_idx], cfg=cfg, top_k=top_k,
                                        verse_name=verse_name, min_score=min_score,
                                        exclude_run_ids=exclude_run_ids, decay_lambda=decay_lambda,
    ...
    final_results: List[List[ScenarioMatch]] = []
    for query_results in all_results:
        seen = set()
        unique = []
        for match in query_results:
            key = (match.run_id, match.episode_id, match.step_idx)
            if key not in seen:
                seen.add(key)
                unique.append(match)
