...
class TestDTMemory(unittest.TestCase):
    def test_memory_hooks(self):
        with tempfile.TemporaryDirectory() as td:
            # Let's create a dummy DT checkpoint
            d_model = 16
            from models.decision_transformer import DecisionTransformerConfig, DecisionTransformer
            cfg = DecisionTransformerConfig(
                state_dim=4, action_dim=2, context_len=2, d_model=d_model, n_head=1, n_layer=1, max_timestep=100
            )
            ...
            agent = TransformerAgent(spec=spec, observation_space=SpaceSpec("dict"), action_space=SpaceSpec("discrete", n=2))
