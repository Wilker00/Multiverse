...
class TestTrainerOnDemandMemoryRecall(unittest.TestCase):
    ...
    def test_memory_recall_agent_can_filter_same_verse_by_policy_bias(self):
        with tempfile.TemporaryDirectory() as td:
            runs_root = os.path.join(td, "runs")
            mem_root = os.path.join(td, "central_memory")
            os.makedirs(runs_root, exist_ok=True)
            ...
            rows = [
                {
                    "run_id": "memory_transfer",
                    "episode_id": "ep_transfer",
                    "step_idx": 0,
                    "t_ms": 1,
                    "verse_name": "line_world",
                    "policy_id": "transfer_bias",
                    "obs": obs0,
                    "obs_vector": obs_to_vector(obs0),
                    "action": 1,
                    "reward": 1.0,
                ...
                {
                    "run_id": "memory_baseline",
                    "episode_id": "ep_baseline",
                    "step_idx": 0,
                    "t_ms": 2,
                    "verse_name": "line_world",
                    "policy_id": "baseline_bias",
                    "obs": obs0,
                    "obs_vector": obs_to_vector(obs0),
                    "action": 0,
                    "reward": 1.0,
            ...
            agent = AgentSpec(
                spec_version="v1",
                policy_id="memory_recaller_bias",
                policy_version="0.1",
                algo="memory_recall",
                seed=7,
                config={
                    ...
                    "recall_same_verse_only": True,
                    "recall_vote_weight": 1.0,
                    "recall_policy_ids": ["transfer_bias"],
                    "recall_exclude_policy_ids": ["baseline_bias"],
                },
            )
            ...
            info = event.get("info", {})
            action_info = (info.get("action_info") or {})
            bundle = action_info.get("memory_recall_pointer", "")
            self.assertEqual(int(event.get("action", -1)), 1)
            self.assertIn("memory_transfer", str(bundle))
            transfer_records = info.get("transfer_decision_records") or []
            self.assertTrue(bool(transfer_records))
            source = (transfer_records[0] or {}).get("source", {})
            self.assertEqual(str(source.get("policy_id", "")), "transfer_bias")

    def test_memory_bootstrap_prefetches_before_first_action(self):
        with tempfile.TemporaryDirectory() as td:
            runs_root = os.path.join(td, "runs")
            mem_root = os.path.join(td, "central_memory")
            os.makedirs(runs_root, exist_ok=True)
            ...
            agent = AgentSpec(
                spec_version="v1",
                policy_id="memory_bootstrap_recaller",
                policy_version="0.1",
                algo="memory_recall",
                seed=7,
                config={
            ...
            action_info = info.get("action_info") or {}
            self.assertTrue(bool(action_info.get("memory_recall_used", False)))
            self.assertIn("runs/memory_bootstrap/events.jsonl", str(action_info.get("memory_recall_pointer", "")))
